...
```mermaid
flowchart TD
    A["You encounter<br>a plant claim"] --> B{"Who is the<br>source?"}

    B -->|"Peer-reviewed journal,<br>University Extension,<br>Gov. agency"| T1["Tier 1:<br>Strongest"]
    B -->|"Credentialed author,<br>established conservation<br>organization"| T2["Tier 2:<br>Generally Reliable"]
    B -->|"Blog, social media,<br>nursery marketing"| T3["Tier 3:<br>Use with Caution"]
    B -->|"Anonymous post,<br>ad, viral content"| T4["Tier 4:<br>Treat with Skepticism"]

    T1 --> C1["Accept with<br>normal scrutiny"]
    T2 --> C2["Verify key claims<br>against Tier 1 sources"]
    ...
    style T3 fill:#FFE0B2,color:#000
...
```mermaid
flowchart TD
    A["You see a claim<br>about native plants"] --> B{"Does it trigger a<br>strong emotional<br>response?"}

    B -->|"Yes"| C["PAUSE - Emotional<br>reactions signal<br>need to verify"]
    B -->|"No"| D{"Does the source<br>cite evidence?"}

    C --> D

    D -->|"No citations"| E["Treat as<br>unverified"]
    D -->|"Yes, cites sources"| F{"Can you find the<br>same claim in 2+<br>independent sources?"}

    F -->|"No"| G["Claim may be<br>isolated or false"]
    F -->|"Yes"| H{"Is the research<br>recent and relevant<br>to your region?"}

    H -->|"No - outdated<br>or wrong region"| I["Apply with<br>caution"]
    H -->|"Yes"| J["Claim is likely<br>reliable - act on it"]

    E --> K["Check U of MN<br>Extension or<br>Google Scholar"]
    G --> K

    style A fill:#B2DFDB,color:#000
