...
<details markdown="1">
<summary>Leaf Shape Identifier MicroSim</summary>
Type: microsim
...
```mermaid
flowchart LR
    A[Unknown Flower] --> B{Flower<br>Symmetry?}
    B -->|Composite Head<br>Ray + Disc| C[Asteraceae<br>Daisy Family]
    B -->|Two-Lipped,<br>Square Stem| D[Lamiaceae<br>Mint Family]
    B -->|Banner + Wings<br>+ Keel| E[Fabaceae<br>Pea Family]
    B -->|Flat Umbel<br>of Tiny Flowers| F[Apiaceae<br>Carrot Family]
    C --> G{Bloom Season?}
    D --> G
    E --> G
    F --> G
    G -->|Apr - May| H[Spring:<br>Hepatica, Pasque Flower]
    G -->|Jun - Jul| I[Early Summer:<br>Black-Eyed Susan, Lupine]
    G -->|Aug - Sep| J[Late Summer:<br>Blazing Star, Joe-Pye Weed]
    G -->|Sep - Oct| K[Fall:<br>Asters, Gentians]
```
...
```mermaid
flowchart TD
    A[Observe the Plant] --> B{Woody or<br>Non-Woody?}
    B -->|Woody| C{Single Trunk<br>or Multiple Stems?}
    C -->|Single Trunk| D[Tree]
    C -->|Multiple Stems| E[Shrub]
    B -->|Non-Woody| F{Leaf Type?}
    F -->|Broad Leaves| G[Forb / Wildflower]
    F -->|Narrow, Parallel Veins| H{Stem Shape?}
    H -->|Round, Hollow<br>with Nodes| I[Grass]
    H -->|Triangular,<br>Solid| J[Sedge]
    H -->|Round, Solid<br>No Nodes| K[Rush]
    D --> L[Check Leaf Arrangement<br>Opposite vs. Alternate]
    E --> L
    G --> L
    I --> M[Examine Ligule,<br>Sheath, Inflorescence]
    J --> M
    L --> N[Examine Flowers<br>or Fruits]
    M --> N
    N --> O[Consult Field Guide<br>or ID App]
    O --> P[Record Observation<br>on iNaturalist]
```
